...
__name(toStationSummary, "toStationSummary");
...
operatorRoutes.patch("/print-jobs/:id/status", async (context) => {
  ...
  if (!body || !body.status) {
    return context.json({ error: "Missing status payload." }, 400);
  }
  ...
  if (result.meta.changes === 0) {
    return context.json({ error: "Print job not found." }, 404);
  }
...
operatorRoutes.get("/print-jobs/:id/file", async (context) => {
  ...
  if (!job || !job.outputR2Key) {
    return context.json({ error: "File not found for this print job." }, 404);
  }
  const file = await context.env.ASSETS.get(job.outputR2Key);
  if (!file) {
    return context.json({ error: "File not found in storage." }, 404);
  }
...
publicRoutes.get("/stations/:slug", async (context) => {
  ...
  if (!row) {
    return context.json({ error: `Station ${slug} was not found.` }, 404);
  }
  return context.json({
...
publicRoutes.post("/print-jobs", async (context) => {
  ...
  if (typeof stationSlug !== "string" || typeof templateId !== "string" || typeof slotCountRaw !== "string" || !(file instanceof File)) {
    return context.json({ error: "Invalid print job payload." }, 400);
  }
  const slotCount = Number(slotCountRaw);
  const totalAmount = Number(totalAmountRaw ?? 0);
  if (!Number.isFinite(slotCount) || slotCount <= 0) {
    return context.json({ error: "slotCount must be a positive number." }, 400);
  }
  const stationRow = await context.env.DB.prepare(
    "SELECT id, slug, name, location, status FROM stations WHERE slug = ? LIMIT 1"
  ).bind(stationSlug).first();
  if (!stationRow) {
    return context.json({ error: `Station ${stationSlug} was not found.` }, 404);
  }
  if (stationRow.status !== "active") {
    return context.json({ error: `Station ${stationSlug} is inactive.` }, 409);
  }
  ...
  await context.env.DB.prepare(
    `
      ...
      VALUES (?, ?, ?, 'pending', ?, ?, ?, ?)
    `
  ).bind(jobId, stationRow.id, jobCode, totalAmount, r2Key, templateId, slotCount).run();
  await context.env.DB.prepare(
    `
      ...
      VALUES (?, ?, ?, ?, 0, ?)
    `
  ).bind(invoiceId, jobId, invoiceCode, totalAmount, totalAmount).run();
  const job = {
    id: jobId,
...
// .wrangler/tmp/bundle-5bT9oW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-5bT9oW/middleware-loader.entry.ts
